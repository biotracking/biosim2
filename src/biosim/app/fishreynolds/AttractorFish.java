...
public class AttractorFish implements Agent{
	...
	public AttractorFish(AbstractFish b){
		fishBody = b;
	}
	public void init(){
	}
	...
	public void act(double time){
		...
		MutableDouble2D att = new MutableDouble2D(0.0,0.0);
		...
		fishBody.getPoiDir(att,"attractor");
		...
		double desiredTVel = (-20.0*sep.y) + (0.1*ori.y) + (0.4*coh.y) + (-20.0*wall.y) + (0.1*att.y);
	...
	public static void main(String[] args){
		try{
			...
			cmdlnDefaults.setProperty("--attractor","true");
			...
			if(cmdlnArgs.getProperty("--attractor").equalsIgnoreCase("true")){
				env.addStaticPOI("attractor",WIDTH/2.0, HEIGHT/2.0);
			}
			...
			for(int i=0; i< numFish;i++){
				agents[i] = new AttractorFish(bodies[i]);
